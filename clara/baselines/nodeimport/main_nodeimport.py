# main_nodeimport.py (refatorado)

import os
import random
import torch
import torch.nn.functional as F
from nodeimport.data_utils import *
from nodeimport.nets import *
from nodeimport.imp_calc import *
...
def get_default_args():
    ...
    parser.add_argument("--running-time", type=int, default=1)
    ...
    # NodeImport-specific
    parser.add_argument("--meta-size", type=int, default=1)
    parser.add_argument(
        "--context-emb", type=str, choices=["SSGC", "APPNP"], default="APPNP"
    )
    ...
    # KMedoids
    parser.add_argument("--km-depth", type=int, default=8)
    parser.add_argument("--km-alpha", type=float, default=0.1)
    parser.add_argument("--km-method", type=str, default="pam")
    parser.add_argument(
        "--km-metric",
        type=str,
...
# =========================
# Função reutilizável por fora
# =========================


def run_nodeimport(data, args):
    # todo o conteúdo atual do main.py, exceto:
    # - parser.parse_args()
    # - get_dataset
    # - print(args)
    # deve ser copiado para dentro desta função

    # Dica: inicie com
    device = torch.device(f"cuda:{args.device}" if torch.cuda.is_available() else "cpu")
    data = data.to(device)
    n_cls = data.y.max().item() + 1
    # print(f"Total num of classes: {n_cls}")
    ...
    def test():
        ...
        for i, mask in enumerate([data_train_mask, data_val_mask, data_test_mask]):
            ...
            probs_bin = probs[mask][:, 1].cpu().numpy()
            auc = roc_auc_score(y_true, probs_bin)
    ...
    (
        ...
        avg_test_f1,
        avg_test_auc,
        avg_test_per_class_f1s,
    ) = ([], [], [], [], [], [])
    for r in range(repeatition):
        if args.dataset in ["Cora", "CiteSeer", "PubMed"]:
            data_train_mask, data_val_mask, data_test_mask = (
                data.train_mask[:, r % 10].clone().bool(),
                data.val_mask[:, r % 10].clone().bool(),
                data.test_mask[:, r % 10].clone().bool(),
            )
        else:
            data_train_mask, data_val_mask, data_test_mask = (
                split_for_Amazon_natural_LT(data.y.clone(), n_cls, args.imb_ratio)
            )
        ...
        if args.dataset in ["Cora", "CiteSeer", "PubMed"]:
            ...
            (
                ...
                data.edge_index, data.y, n_data, n_cls, 1.0, data_train_mask.clone()
            )
        ...
        if args.context_emb == "SSGC":
            P_KM = SSGC_Embedding(
                data.x,
                data.edge_index[:, train_edge_mask],
                k=args.km_depth,
                alpha=args.km_alpha,
            )
        elif args.context_emb == "APPNP":
            P_KM = APPNP_Embedding(
                data.x,
                data.edge_index[:, train_edge_mask],
                k=args.km_depth,
                alpha=args.km_alpha,
        ...
        for i in range(n_cls):
            ...
            # print(f"Classe {i}: {tmp_num_nodes} nós")

            class_idx_numpy = class_idx_tensor.cpu().numpy()

            train_emb = P_KM[class_idx_numpy]
            tmp_kmedoids = KMedoids(
                n_clusters=args.meta_size,
                method=args.km_method,
                metric=args.km_metric,
                init=args.km_init,
            ).fit(train_emb)
        ...
        in_neighbor_dist = get_in_neighbor_dist(
            data.edge_index[:, train_edge_mask], data.x.size(0)
        )
        ...
        for epoch in range(1, 2001):
            if epoch > 1:
                with torch.no_grad():
                    ...
                    # Synthesize nodes
                    center_idx, aux_idx = sample_synthesis_pair(
                        class_num_tensor, idx_info, aux_pool_idx, aux_pool_label
                    )
                    lambdas = beta.sample((center_idx.size(0), 1)).to(device)
                    new_edge_index = sample_neighbors(
                        data.edge_index[:, train_edge_mask],
                        data.x.size(0),
                        in_neighbor_dist,
                        center_idx,
                    ...
                    Y_mixed = lambdas * F.one_hot(
                        data.y[center_idx], num_classes=n_cls
                    ) + (1 - lambdas) * F.one_hot(data.y[aux_idx], num_classes=n_cls)
                    ...
                    tmp_data_unlabeled_mask = torch.cat(
                        [data_unlabeled_mask, not_syn_mask]
                    )

                    tmp_data_meta_mask = subsample_meta_mask(
                        meta_idx_info,
                        int(args.meta_size * (1 - args.meta_drop)),
                        data.x.size(0),
                    )
                    tmp_data_meta_mask = tmp_data_meta_mask.to(device)
                    tmp_data_meta_mask = torch.cat([tmp_data_meta_mask, not_syn_mask])

                    tmp_data_syn_mask = torch.cat(
                        [
                            torch.zeros_like(data_train_mask).bool(),
                    ...
                    P = context_embedding(
                        new_x, new_edge_index, k=args.depth, alpha=args.alpha
                    )
                    output = model(new_x, new_edge_index, None)
                    S = F.softmax(output, dim=1)

                    # for training nodes
                    individual_importance = calculate_importance(
                        P[tmp_data_train_mask],
                        P[tmp_data_meta_mask],
                        S[tmp_data_train_mask],
                        S[tmp_data_meta_mask],
                        new_Y[tmp_data_train_mask],
                        new_Y[tmp_data_meta_mask],
                    )
                    filtering_mask_train = individual_importance > 0.0

                    # for unlabeled nodes
                    pesudo_y = torch.max(S, dim=1)[1]
                    pesudo_Y = F.one_hot(pesudo_y, num_classes=n_cls)
                    individual_importance_unlabeled = calculate_importance(
                        P[tmp_data_unlabeled_mask],
                        P[tmp_data_meta_mask],
                        S[tmp_data_unlabeled_mask],
                        S[tmp_data_meta_mask],
                        pesudo_Y[tmp_data_unlabeled_mask],
                        new_Y[tmp_data_meta_mask],
                    )
                    filtering_mask_unlabeled = individual_importance_unlabeled > 0.0

                    # for synthetic nodes
                    individual_importance_syn = calculate_importance(
                        P[tmp_data_syn_mask],
                        P[tmp_data_meta_mask],
                        S[tmp_data_syn_mask],
                        S[tmp_data_meta_mask],
                        new_Y[tmp_data_syn_mask],
                        new_Y[tmp_data_meta_mask],
                    )
                    filtering_mask_syn = individual_importance_syn > 0.0
            ...
            loss = F.cross_entropy(
                output[tmp_data_train_mask][filtering_mask_train],
                new_y[tmp_data_train_mask][filtering_mask_train],
            )

            # loss for synthetic nodes
            if filtering_mask_syn is not None:
                syn_loss_1 = lambdas[filtering_mask_syn, 0] * F.cross_entropy(
                    output[tmp_data_syn_mask][filtering_mask_syn],
                    data.y[center_idx][filtering_mask_syn],
                    reduction="none",
                )
                syn_loss_2 = (1 - lambdas[filtering_mask_syn, 0]) * F.cross_entropy(
                    output[tmp_data_syn_mask][filtering_mask_syn],
                    data.y[aux_idx][filtering_mask_syn],
                    reduction="none",
                )
            ...
            if pesudo_y is not None and epoch > args.ul_warmup:
                tmp_scale = (
                    ul_scale_schedule(epoch, args.ul_decay_epoch) * args.ul_scale
                )
                ul_loss = F.cross_entropy(
                    output[tmp_data_unlabeled_mask][filtering_mask_unlabeled],
    ...
    model.eval()
    with torch.no_grad():
        final_logits = model(data.x, data.edge_index[:, train_edge_mask], None)
        probs = F.softmax(final_logits, dim=1)

        data.y_pred = final_logits.argmax(dim=1)  # classe prevista
        data.y_prob = probs[:, 1]

    # Agora todo o resto segue igual: masks, kmedoids, modelo, treino, etc.

    # No fim, pode retornar o modelo ou métricas:
    return {
        "model": model,
        # "graph": data,
        # "best_val_acc_f1": best_val_acc_f1,
        # "test_f1": test_f1,
...
# =========================
# Suporte para execução standalone
# =========================
