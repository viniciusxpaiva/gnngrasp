...
def get_parser():
    ...
    parser.add_argument(
        "--epochs", type=int, default=1010, help="Number of epochs to train."
    )
    ...
    parser.add_argument(
        "--batch_nums", type=int, default=6000, help="number of batches per epoch"
    )
    parser.add_argument(
        "--batch_size", type=int, default=40, help="number of batches per epoch"
    )
    ...
    parser.add_argument(
        "--opt_new_G", action="store_true", default=False
    )  # whether optimize the decoded graph based on classification result.
    ...
    parser.add_argument(
        "--model", type=str, default="sage", choices=["sage", "gcn", "GAT"]
    )
...
def print_edges_num(dense_adj, labels):
    ...
    for i in range(c_num):
        for j in range(c_num):
            ...
            # print(
            #    "edges between class {:d} and class {:d}: {:f}".format(i, j, edge_num)
            # )
...
def print_class_acc(output, labels, class_num_list, pre="valid"):
    ...
    if labels.max() > 1:
        auc_score = roc_auc_score(
            labels.detach(),
            F.softmax(output, dim=-1).detach(),
            average="macro",
            multi_class="ovr",
        )
    else:
        auc_score = roc_auc_score(
            labels.detach(), F.softmax(output, dim=-1)[:, 1].detach(), average="macro"
        )

    macro_F = f1_score(
        labels.detach(), torch.argmax(output, dim=-1).detach(), average="macro"
    )
...
def src_smote(adj, features, labels, idx_train, portion=1.0, im_class_num=3):
    ...
    for i in range(im_class_num):
        ...
        interp_place = random.random()
        embed = (
            chosen_embed + (chosen_embed[idx_neighbor, :] - chosen_embed) * interp_place
        )
