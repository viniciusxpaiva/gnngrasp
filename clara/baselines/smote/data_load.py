...
def load_data_protein(dataset_name):
    # print(f"[!] Carregando dataset de: smote/training_data/protein/{dataset_name}.npz")
    data = np.load(
        f"/home/vinicius/Desktop/deep-grasp/smote/templates/{dataset_name}_training_data.npz",
        allow_pickle=True,
    )

    features = data["features"]  # shape: [n_nodes, n_features]
    adj = data["adj"].item()  # scipy sparse matrix
    features = torch.FloatTensor(features)
    # adj = sparse_mx_to_torch_sparse_tensor(adj)

    labels = data["labels"]  # shape: [n_nodes]
    labels = torch.LongTensor(labels)
    return adj, features, labels

    # smote.utils.print_edges_num(adj.to_dense(), labels)

    # os.makedirs(
    #    f"/home/vinicius/Desktop/graph-smote/checkpoint/protein/{dataset_name}",
    #    exist_ok=True,
    # )


def load_input_protein(output_folder):
    data = np.load(
        f"{output_folder}/input_training_data.npz",
        allow_pickle=True,
    )
...
def Extract_graph(edgelist, fake_node, node_num):
    ...
    adj_1 = sp.coo_matrix(
        (np.ones(len(edgelist)), (edgelist[:, 0], edgelist[:, 1])),
        shape=(edgelist.max() + 1, edgelist.max() + 1),
        dtype=np.float32,
    )
...
def load_data_twitter():
    ...
    adj = np.loadtxt(
        adj_path, delimiter=",", skiprows=1
    )  # (total: 16011444 edges, 5384162 nodes)
    adj = adj.astype(int)
    adj = np.array(adj, dtype=int)
    fake_node = np.genfromtxt(
        fake_id_path, delimiter=",", skip_header=1, usecols=(0), dtype=int
    )  # (12437)
...
def norm_sparse(
    adj,
):  # normalize a torch dense tensor for GCN, and change it into sparse.
...
def find_shown_index(adj, center_ind, steps=2):
    ...
    for step in range(steps):
        new_start_point = []
        candid_point = set(
            adj[start_point, :].reshape(-1, adj.shape[1]).nonzero()[:, 1]
        )
