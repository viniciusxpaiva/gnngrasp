...
import utils
import data_load
from data_load import load_data_protein, sparse_mx_to_torch_sparse_tensor
import random
import ipdb
import copy
...
# npz_path = "/home/vinicius/Desktop/deep-grasp/data/smote_data/1a8t_A_training_data.npz"
...
if args.dataset == "cora":
    ...
    # Defina aqui o caminho do seu .npz ou leia de args se quiser tornar isso mais flexível
    adj, features, labels = load_data_protein(args.dataset)
    adj = sparse_mx_to_torch_sparse_tensor(adj)
    im_class_num = 1  # ajuste conforme o número de classes desbalanceadas
    class_sample_num = 20
# else:
...
for i in range(labels.max().item() + 1):
    if (
        args.imbalance and i > labels.max().item() - im_class_num
...
if args.setting == "upsampling":
    adj, features, labels, idx_train = utils.src_upsample(
        adj,
        features,
        labels,
        idx_train,
...
if args.setting == "smote":
    adj, features, labels, idx_train = utils.src_smote(
        adj,
        features,
        labels,
        idx_train,
...
if args.setting != "embed_up":
    if args.model == "sage":
        encoder = models.Sage_En(
            nfeat=features.shape[1],
            nhid=args.nhid,
            nembed=args.nhid,
            dropout=args.dropout,
        )
        classifier = models.Sage_Classifier(
            nembed=args.nhid,
            nhid=args.nhid,
        ...
        encoder = models.GCN_En(
            nfeat=features.shape[1],
            nhid=args.nhid,
            nembed=args.nhid,
            dropout=args.dropout,
        )
        classifier = models.GCN_Classifier(
            nembed=args.nhid,
            nhid=args.nhid,
            nclass=labels.max().item() + 1,
            dropout=args.dropout,
        ...
        encoder = models.GAT_En(
            nfeat=features.shape[1],
            nhid=args.nhid,
            nembed=args.nhid,
            dropout=args.dropout,
        )
        classifier = models.GAT_Classifier(
            nembed=args.nhid,
            nhid=args.nhid,
            nclass=labels.max().item() + 1,
            dropout=args.dropout,
...
def train(epoch):
    ...
    if (
        args.setting == "recon_newG"
        or args.setting == "recon"
        or args.setting == "newG_cls"
    ):
        ...
        loss_rec = utils.adj_mse_loss(
            generated_G[:ori_num, :][:, :ori_num], adj.detach().to_dense()
        )
        ...
        if not args.opt_new_G:
            ...
            # ipdb.set_trace()
            edge_ac = adj_new[:ori_num, :ori_num].eq(adj.to_dense()).double().sum() / (
                ori_num**2
            )
        else:
            adj_new = generated_G
            edge_ac = F.l1_loss(
                adj_new[:ori_num, :ori_num], adj.to_dense(), reduction="mean"
            )

        # calculate generation information
        exist_edge_prob = adj_new[
            :ori_num, :ori_num
        ].mean()  # edge prob for existing nodes
        generated_edge_prob = adj_new[
            ori_num:, :ori_num
        ].mean()  # edge prob for generated nodes
        # print(
        #    "edge acc: {:.4f}, exist_edge_prob: {:.4f}, generated_edge_prob: {:.4f}".format(
        ...
        # print(
        #    "after filtering, edge acc: {:.4f}, exist_edge_prob: {:.4f}, generated_edge_prob: {:.4f}".format(
        #        edge_ac.item(), exist_edge_prob.item(), generated_edge_prob.item()
        #    )
        # )
        ...
        embed, labels_new, idx_train_new = utils.recon_upsample(
            embed, labels, idx_train, portion=args.up_scale, im_class_num=im_class_num
        )
        adj_new = adj
    else:
        labels_new = labels
    ...
    if args.setting == "reweight":
        ...
        loss_train = F.cross_entropy(
            output[idx_train_new], labels_new[idx_train_new], weight=weight
        )
    else:
...
def test(epoch=0):
    ...
    utils.print_class_acc(
        output[idx_test], labels[idx_test], class_num_mat[:, 2], pre="test"
    )
...
def save_model(epoch):
    ...
    torch.save(
        saved_content,
        "smote/checkpoint/{}/{}_{}_{}_{}.pth".format(
            args.dataset, args.setting, epoch, args.opt_new_G, args.im_ratio
        ),
    )
...
def load_model(filename):
    loaded_content = torch.load(
        "smote/checkpoint/{}/{}.pth".format(args.dataset, filename),
