...
from baselines.BAT.bat import BatAugmenter
...
class BATPrediction:
    ...
    def prepare_dataset(self, input_protein):
        ...
        for d in raw_data_list:
            if d is None:
                continue
            try:
                # Add fake masks (required by BAT)
                d.train_mask = torch.ones(d.x.size(0), dtype=torch.bool)  # fake mask
                d.val_mask = torch.zeros(d.x.size(0), dtype=torch.bool)
                d.test_mask = torch.zeros(d.x.size(0), dtype=torch.bool)

                # Move to same device as model
                d = d.to(self._device)

                # Initialize and apply BAT
                augmenter = BatAugmenter(mode="bat1").init_with_data(d)
                x_aug, edge_index_aug, _ = augmenter.augment(
                    self.model, d.x, d.edge_index
                )

                # Adapt labels to match augmented nodes
                d.y, d.train_mask = augmenter.adapt_labels_and_train_mask(
                    d.y, d.train_mask
                )

                # Update data object
                d.x = x_aug
                d.edge_index = edge_index_aug

                data_list.append(d)
            except Exception as e:
                print(
                    f"[WARN] Skipping {getattr(d, 'template_id', '[unknown]')} due to BAT error: {e}"
                )
    ...
    def _prepare_inputs(self, input_protein):
        ...
        # --- Required ---
        x = prepare_node_tensor(embeddings_df).to(self._device)
    ...
    def _print_graph_debug_info(self, data, residue_ids, max_nodes=3):
        ...
        print(
            f"         x.shape={data.x.shape}, x_properties.shape={data.x_properties.shape}, edge_index.shape={data.edge_index.shape}, y.shape={data.y.shape}"
        )
